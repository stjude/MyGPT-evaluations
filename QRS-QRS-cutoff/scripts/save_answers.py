    ...
    for model in MODELS:
        ...
        with open(os.path.realpath(output_csv_path), 'w', buffering=1) as f:
            ...
            for j, (question, answer, contexts, q_id) in enumerate(zip(question_list, answers_list, context_lists, id_list)):
                ...
                if contexts == []:
                    f.write(f"{q_id},n/a,n/a,n/a,n/a,n/a,n/a,n/a\n")
                    continue
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        hal_idx_by_ml = data.get('hallucination_index_by_ml', '')
                        hal_idx_by_eq = data.get('hallucination_index_by_equation', '')
                        ...
                        f.write(f"{q_id},{m_dist},{rel_score},{hal_idx_by_ml},{hal_idx_by_eq},\"{vector_distances}\",\"{vector_scores}\",\"{bm25_scores}\",\"{rerank_sentiments}\"\n")
                        f.flush() 
                        success_count += 1
